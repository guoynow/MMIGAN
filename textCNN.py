    ...
    def train_model(self, x_train, tokenizer):
        ...
        for epoch in range(self.epochs):
            ...
            for i in range(batch_no):
                ...
                batch_x = utilss.get_batch(x_train,self.batch_size,i,batch_no)
                batch_x = tf.reshape(batch_x, shape=(batch_x.shape[0], batch_x.shape[1], 1,1))
                with tf.GradientTape() as en_tape, tf.GradientTape() as de_tape, tf.GradientTape() as emb_tape:
                    
                    emb = batch_x
                    norm_emb1 = tf.reshape(emb,shape=(emb.shape[0],emb.shape[1]*emb.shape[2]*emb.shape[3]))
                    ...
                    self.loss = losss(x_emb_norm, dec_list)
                    ...
                    if (epoch == 9 and i == batch_no - 1):
                        np.savetxt("./x_pred123.txt", renormalization[390], delimiter=',', fmt='%s')
                        np.savetxt("./x_emb123.txt", norm_emb1[390], delimiter=',', fmt='%s')
                    if (epoch == 9):
                        
                        result.append(tokenizer.sequences_to_texts(renormalization))
                        number.append(renormalization)

                if epoch % display_step == 0:
                    print("Epoch:", '%04d' % (epoch + 1), "loss=", "{:.9f}".format(self.loss))
            np.savetxt("./result.txt", result, delimiter=',', fmt='%s')
            np.savetxt("./number.txt", number, delimiter=',', fmt='%s')
