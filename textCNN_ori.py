...
class text_encoder(Model):
    def __init__(self, filter_num, embedding_dims, kernel_regularizer):
        ...
        self.conv1 = Conv2D(filters=filter_num, kernel_size=(2, embedding_dims), activation='elu',
                   kernel_regularizer=kernel_regularizer)
        self.conv2 = Conv2D(filters=filter_num, kernel_size=(3, embedding_dims), activation='elu',
                            kernel_regularizer=kernel_regularizer)
        self.conv3 = Conv2D(filters=filter_num, kernel_size=(4, embedding_dims), activation='elu',
                            kernel_regularizer=kernel_regularizer)
...
class TextCNN(Model):
    ...
    def train_model(self, x_train):
        ...
        _train_data = utilss.DataSet_Double(x_train)
        ...
        for epoch in range(self.epochs):
            ...
            for i in range(batch_no):
                batch_x = _train_data.next_batch(batch_size=self.batch_size)
                print(batch_x.shape)
                batch_x = tf.reshape(batch_x, shape=(batch_x.shape[0], batch_x.shape[1], 1))

                with tf.GradientTape() as en_tape, tf.GradientTape() as de_tape, tf.GradientTape() as emb_tape:
                    emb = self.text_embedding(batch_x)
                    norm_emb = tf.reshape(emb,shape=(emb.shape[0],emb.shape[1]*emb.shape[2]*emb.shape[3]))
                    norm_emb, parameters = utilss.normalization(norm_emb)
                    norm_emb = tf.reshape(norm_emb,shape=(emb.shape[0],emb.shape[1],emb.shape[2],emb.shape[3]))
                    print("emb::", emb.shape)
                    enc_list= self.en(norm_emb)
                    dec_list = self.de(enc_list)
                    dec_list1 = tf.reshape(dec_list,shape=(dec_list.shape[0],dec_list.shape[1]*dec_list.shape[2]))

                    x_emb_norm = tf.reshape(norm_emb, shape=[-1, self.maxlen,
                                                   self.embedding_dims])
                    dec_list1 = tf.reshape(dec_list1,shape=[-1, self.maxlen,
                                                   self.embedding_dims])

                    self.loss= tf.reduce_mean((x_emb_norm - dec_list1) ** 2)

                    self.text_embedding_gradients = emb_tape.gradient(self.loss, self.text_embedding.trainable_variables)
                    self.emb_optimizer.apply_gradients(zip(self.text_embedding_gradients, self.text_embedding.trainable_variables))

                    self.en_gradients = en_tape.gradient(self.loss, self.en.trainable_variables)
                    self.en_optimizer.apply_gradients(zip(self.en_gradients, self.en.trainable_variables))

                    self.de_gradients = de_tape.gradient(self.loss, self.de.trainable_variables)
                    self.de_optimizer.apply_gradients(zip(self.de_gradients, self.de.trainable_variables))
                    if (epoch == self.batch_size-1):
                        np.savetxt("./x_emb123.txt", x_emb_norm[2], delimiter=',', fmt='%s')
                        np.savetxt("./x_pred123.txt", dec_list1[2], delimiter=',', fmt='%s')
    ...
    def loss_optimizer(self, x_emb, x_pred):
        self.loss_ae = tf.reduce_mean((x_emb - x_pred) ** 2)
        self.loss = self.loss_ae

    def save_model(self):
        self.textM.save("./model/text_cnn.h5", save_format='tf')
